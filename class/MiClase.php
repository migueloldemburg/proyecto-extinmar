...
class MiClase{
    ...
    function restar_de_almacen($id, $cantidad){
        if($this->obtener_servicio_by_id($id)){
            if($this->tipo == 'servicio'){
                ...
                //si ha alcanzado la cantidad minima, manda email con informacion
                if($nuevaCantidad <= 10){
                    $usuario = new Usuarios();
                    $usuario->sendNotificationStock($this->nombre);
                }
                return $this->set_nueva_cantidad($nuevaCantidad.$unidad);
            }elseif($this->tipo == 'repuesto'){
                $stock = preg_replace('/[^0-9]+/', '', $this->cantidad);
    ...
    function obtener_extintor_almacen($desde, $hasta, $id_ubicacion){
        ...
        if($id_ubicacion==0){
            $select = " SELECT ne.id, ne.estado AS estadoExtintor, ne.observacion, ec.nombre AS categoria, e.capacidad, sc.nombre AS servicio, n.fecha_registro, n.fecha_entrega, u.departamento, u.pasillo
